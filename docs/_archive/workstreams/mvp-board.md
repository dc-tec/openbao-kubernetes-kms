...
- WS11-T06D provider binary upgrade/rollback coverage is implemented through `make test-e2e-provider-upgrade-rollback-openbao-ci`; it builds distinct old/new provider images and verifies old/new ciphertext readback across old -> new -> old provider restarts.
...
- WS11-T09B sustained direct decrypt soak coverage is implemented through `make test-e2e-provider-decrypt-soak-openbao-ci`; it prepares a fixed encrypted sample corpus, sustains concurrent direct decrypts through real provider/OpenBao, and checks error, p95 latency, memory, and PID growth bounds.
