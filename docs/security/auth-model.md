...
## Rationale: JWT Auth, Not Kubernetes-As-OIDC

OpenBao JWT auth is preferred over OpenBao Kubernetes auth for this plugin.

OpenBao's JWT and OIDC auth method verifies JWTs cryptographically using configured local keys, JWKS, or OIDC discovery. OpenBao's Kubernetes-as-OIDC guidance documents that JWT auth does not call Kubernetes TokenReview, while Kubernetes auth does. The tradeoff is that revoked tokens remain valid until expiry when only cryptographic JWT validation is used.
...
| Kubernetes-issued ServiceAccount JWT from the protected cluster | Supported with caution | Kubernetes ServiceAccount JWTs carry issuer, subject, audience, and expiry claims and validate offline through discovery. Offline validation does not prove that bound objects still exist. Renewal may depend on kubelet and API server behavior. |
| Long-lived static JWT or token on disk | Emergency or constrained environments only | Operationally robust, weaker security. Should not be the default. Use response wrapping for initial distribution where practical. |
...
Response wrapping is not part of the hot path. It is useful for initial delivery of a fallback static credential, for emergency recovery material, or for one-time bootstrap secret handoff. OpenBao response wrapping stores a response behind a single-use wrapping token with a TTL, which can detect mishandling during the handoff.
