...
| Registry state missing | State file removed or first startup after restore | Provider may need to rebuild snapshots before serving | `doctor`, `rotation-plan`, startup logs | Strict state file, rebuild from trusted Transit metadata | Restore state file or rebuild only when metadata completeness is proven | Possible | No |
| Registry state corrupt or rolled back | Disk corruption, unsafe restore, replayed file | Startup or probe fails closed | State load errors, hash mismatch, rollback error | State hash chain, permission checks, monotonic generation | Restore last valid state file or perform documented DR rebuild | Yes | No |
