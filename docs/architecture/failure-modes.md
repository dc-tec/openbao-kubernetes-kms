...
| Registry state missing | State file removed or first startup after restore | Provider may need to rebuild snapshots before serving | `doctor`, `rotation-plan`, startup logs | Strict state file and checkpoint; auto-bootstrap only from initial Transit metadata | Restore state file and checkpoint from backup or a known-good peer; after rotation, current preview releases fail closed rather than rebuilding state | Possible | No |
