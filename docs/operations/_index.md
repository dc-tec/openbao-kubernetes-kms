...
1. [Rotation](/operations/rotation/) to rotate the OpenBao Transit key version, observe provider promotion, migrate Kubernetes resources, and keep old versions decryptable until backup retention allows retirement.
2. [Disaster Recovery](/operations/disaster-recovery/) to recover OpenBao, etcd, provider state, auth material, and control-plane nodes as compatible sets.
