...
E2E_PROVIDER_CERTAUTH_SPIFFE_IMAGE ?= $(IMAGE_REPOSITORY):e2e-certauth-spiffe-$(COMMIT)
E2E_PROVIDER_CERTAUTH_PKCS11_IMAGE ?= $(IMAGE_REPOSITORY):e2e-certauth-pkcs11-$(COMMIT)
E2E_PROVIDER_BUILD ?= true
E2E_KIND_NODE_IMAGE ?= $(shell awk '/^[[:space:]]*kindNodeImage:[[:space:]]*/ { print $$2; exit }' .ci/versions.yaml)
E2E_SPIRE_SERVER_IMAGE ?= $(shell awk '/^[[:space:]]*serverImage:[[:space:]]*/ { print $$2; exit }' .ci/versions.yaml)
E2E_SPIRE_AGENT_IMAGE ?= $(shell awk '/^[[:space:]]*agentImage:[[:space:]]*/ { print $$2; exit }' .ci/versions.yaml)
...
HUGO_VERSION ?= v$(shell tr -d '[:space:]' < .hugo-version)
